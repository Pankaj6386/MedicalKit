...
const EnterLocation = ({ navigation, route }) => {
  ...
  console.log("----route value---", route.params, route.params?.area);
  ...
  console.log(allLocation, "all-- location");
    ...
    if (key == "admin") {
      ...
      // }
        ...
        (route.params?.pic !== "" || imageUri) &&
          ...
          //   uri: route.params?.pic == "" || imageUri || selectedEmoji,
          //   name: "image.jpg",
          //   type: "image/jpeg",
          // }
          payload.append("kit_location_pic", {
        ...
        console.log(
          "----detail report incident --- payload-",
          JSON.stringify(payload)
        );
        setIsLoadings(true);
        dispatch(call_AddLocation_API(payload))
          .then((res) => {
            console.log("update location api status", JSON.stringify(res));
            setIsLoadings(false);
            if (res.payload.status === 200) {
