...
const Signupbusiness = ({ navigation }) => {
  ...
  const handleAddressSubmit = async (address) => {
    getCurrentLocation();
    console.log(address, "---nusiness-add address---", location);
    setCoordinates({ latitude: location?.lat, longitude: location?.lng });
    // try {
    //   const response = await axios.get(
    //     // `https://maps.googleapis.com/maps/api/geocode/json?address=${address}&key=AIzaSyCzwwkQ5_JDq5cNafsXVwPxHuxoGvYKnJU`
  ...
  const handleSkip = () => {
    ...
    dispatch(call_RegisteCompany_Api(payloadValue))
      .then(async (res) => {
        console.log(
          "Call_Register_Company_Api api response in Register Company screen--",
          JSON.stringify(res.payload)
        );

        if (res.payload.status === 200) {
          console.log("register company res.payload.----", res.payload);
          setisLoadings(false);
  ...
  console.log("---registerData---", city, "---", county, "----", country);
  ...
  const handleRegisterCompany = (val) => {
    ...
    const payload = {
      ...
      distributor_email: val.distributor_email,
